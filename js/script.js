...
$(document).ready(function(){ //Start Ready
    ...
    var sdegree = seconds * 6; /* variable declared sdegree = current seconds from computer multipled by 6. ex: seconds=0 0*6=0 which is the starting clock position at 12:00, so if 1*6=6 is the 5th index position on the clock or 6th second counting from 0 is 5. or 12:05 from 12:00 (the 6th second)*/
    ...
    }, 1000); // moves second hand every second "rotates" the clockwise to model the minute hand moves one minute of time after 60seconds is reached and repeats process
    ...
    var mins = new Date().getMinutes(); /* variable declared sdegree = current minutes from computer multipled by 6. ex: minutes=10 10*6=60 degrees or 6 degrees for each increment of a minute from 12 or index of 0, index of 10 == 10 minute position on the clock which is 60 degrees going clockwise.*/
    var hdegree = hours * 30 + (mins / 2); // formula caluclates the hourhand position interms of a degree on the clock from index 0== day's starting hour (o degrees) to index  of 12 == 12th hour (360 degrees) ex: 11:48, 11*30+(48/2)=330+24=354 degrees, hour hand closing in on the 12th hour or 360 degrees.
